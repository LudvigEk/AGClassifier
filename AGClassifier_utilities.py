    ...
    invalid_selection_layout = [
        [
            sg.Text(
                ...
                'Y-limit.\nIt is allowed to select no limit at all for an image, as well as just one X- or Y- limit'),
            sg.Button('OK', size=(8, 4))
        ]
    ]
    return sg.Window("Invalid Selection", invalid_selection_layout)
...
def createInvalidCustomWindow(label, value):
    ...
    return sg.Window("Invalid custom limit", invalid_custom_limit_layout)
...
def createNoSamplePDF():
    ...
    return sg.Window("Missing PDF", missing_file)
